...
public class Granja_PiscicolaDAO {
    ...
    public boolean insertar(GranjaPiscicola finca) {
        ...
        try (PreparedStatement ps = conexion.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setFloat(3, finca.getAltitud());
    ...
    private GranjaPiscicola mapRowToFinca(ResultSet rs) throws SQLException {
        ...
        g.setAltitud(rs.getFloat("longitud"));
        g.setAreaTotal(rs.getFloat("area_total"));
        return g;
    }
    
        // Cuenta cuántas fincas hay registradas (las consideramos "activas")
    public int contarFincasActivas() throws Exception {
        // Reutilizamos el método existente listarTodos()
        java.util.List<com.mycompany.model.GranjaPiscicola> fincas = listarTodos();
        if (fincas == null) {
            return 0;
        }
        return fincas.size();
